...
async function loadSeasons(id) {
  ...
  for (const s of details.seasons.filter(s => s.season_number > 0)) {
    ...
    for (const ep of epsData.episodes) {
      ...
      // ✅ Enregistrement Supabase + redirection
      card.addEventListener('click', async () => {
        const userId = await getUserId();
        const duration = ep.runtime || details.episode_run_time?.[0] || 0;

        if (userId) {
          // --- Enregistrement dans user_progress
          ...
          // --- Incrément des genres dans profile_genres
          if (Array.isArray(details.genres)) {
            const genreUpdates = {};
            details.genres.forEach(g => {
              const col = TMDB_GENRE_NAME_MAP[g.id];
              if (col) genreUpdates[col] = 1;
            });
            if (Object.keys(genreUpdates).length > 0) {
              await supabase.rpc('increment_profile_genres', {
                uid: userId,
...
async function loadFilm() {
  try {
    ...
    // Construction des tags dynamiques (type, note, année, durée)
