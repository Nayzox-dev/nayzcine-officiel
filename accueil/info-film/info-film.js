...
async function getFirstPlayableEpisode(tvId) {
  const details = await fetch(`https://api.themoviedb.org/3/tv/${tvId}?api_key=${apiKey}&language=fr-FR`).then(r=>r.json());
  const seasons = (details.seasons || []).filter(s => s.season_number > 0).sort((a,b)=>a.season_number-b.season_number);
  const firstSeason = seasons[0];
  if (!firstSeason) {
    return { season: 1, episode: 1, runTime: (details.episode_run_time?.[0] || 0), title: details.name };
  }
  const seasonData = await fetch(`https://api.themoviedb.org/3/tv/${tvId}/season/${firstSeason.season_number}?api_key=${apiKey}&language=fr-FR`).then(r=>r.json());
  const firstEp = (seasonData.episodes || [])[0];
  const epNum = firstEp?.episode_number ?? 1;
  const runTime = firstEp?.runtime || details.episode_run_time?.[0] || 0;
  return { season: firstSeason.season_number, episode: epNum, runTime, title: details.name };
}
...
async function loadSeasons(id) {
  ...
  const poster = details.poster_path ? `${IMG_BASE}w500${details.poster_path}` : FALLBACK_IMG;
  ...
  for (const s of details.seasons.filter(s => s.season_number > 0)) {
    ...
    div.innerHTML = `<h3>Saison ${s.season_number}</h3><div class="card-grid"></div>`;
    const grid = div.querySelector('.card-grid');

    const epsData = await fetch(
      `https://api.themoviedb.org/3/tv/${id}/season/${s.season_number}?api_key=${apiKey}&language=fr-FR`
    ).then(r => r.json());

    for (const ep of epsData.episodes) {
      const img = ep.still_path ? `${IMG_BASE}w300${ep.still_path}` : poster;
      const card = document.createElement('div');
      card.className = 'card-film';
      card.innerHTML = `
        <img src="${img}" alt="${ep.name}" onerror="this.src='${poster}'"/>
        <span>${ep.name}</span>`;

      // INSERT conditionnel + redirection (aucun doublon)
      card.addEventListener('click', async () => {
        const userId = await getUserId();
        const duration = ep.runtime || details.episode_run_time?.[0] || 0;
        ...
        if (userId) {
          try {
            const { data: existing, error: selErr } = await supabase
              ...
              .eq('season', s.season_number)
              .eq('episode', ep.episode_number)
              .maybeSingle();
            ...
            if (!selErr && !existing) {
              const { error: insErr } = await supabase.from('user_progress').insert([{
                user_id: userId,
                media_id: Number(id),
                genre: details.genres?.[0]?.id ? (TMDB_GENRE_NAME_MAP[details.genres[0].id] || null) : null,
                is_movie: false,
              ...
              else console.error('Erreur insert user_progress (TV):', insErr);

              if (inserted && Array.isArray(details.genres)) {
                const genreUpdates = {};
                details.genres.forEach(g => { const col = TMDB_GENRE_NAME_MAP[g.id]; if (col) genreUpdates[col] = 1; });
                if (Object.keys(genreUpdates).length > 0) {
                  const { error: rpcErr } = await supabase.rpc('increment_profile_genres', { uid: userId, updates: genreUpdates });
                  if (rpcErr) console.error('Erreur increment_profile_genres (TV):', rpcErr);
                }
              }
            } else if (selErr) {
              console.error('Erreur select user_progress (TV):', selErr);
            }
          } catch(e) { console.error('Exception TV progress insert:', e); }
        }
