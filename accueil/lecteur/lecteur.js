...
// ======= FONCTION loadSource =======
function loadSource(originalUrl) {
  const video = document.querySelector('video');
  const overlay = document.querySelector('.overlay');
  const playPauseBtn = document.querySelector('#playPauseBtn');

  const isM3U8 = /\.m3u8(?:$|\?)/i.test(originalUrl);

  if (isM3U8) {
    // On laisse le serveur réécrire le manifeste
    const proxied = `/api/proxy?url=${encodeURIComponent(originalUrl)}`;
    const hls = new Hls();
    ...
    // Lecture MP4 ou autre format natif
    video.src = `/api/proxy?url=${encodeURIComponent(originalUrl)}`;
...
// Ensuite, ton togglePlayPause() gère le play/pause au clic :
function togglePlayPause() {
  if (video.paused) {
